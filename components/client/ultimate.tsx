 ...
 * yellow accent (#d946ef) reserved for the hero metric only.
 *
  ...
  return (
    <div className="relative rounded-2xl">
      <div
        ...
        style={{
          background:
            "conic-gradient(from 0deg, transparent 0deg, #d946ef 60deg, transparent 140deg)",
      ...
      <div className="relative rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 sm:p-6 lg:p-8 overflow-hidden">
        ...
        {spark && spark.length > 1 && (
          <div className="mt-4">
            <Sparkline data={spark} color="#d946ef" height={36} />
          </div>
        )}
      </div>
  ...
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 sm:p-5 lg:p-6">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" preserveAspectRatio="none" style={{ height }}>
        ...
        <path d={primPath} fill="none" stroke="#d946ef" strokeOpacity="0.7" strokeWidth={1.5} strokeLinejoin="round" />
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
      {ads.map((ad) => {
        ...
        const ringColor =
          score >= 70 ? "#34d399" : score >= 40 ? "#d946ef" : "#fb7185";
        return (
          <div
            key={ad.key}
