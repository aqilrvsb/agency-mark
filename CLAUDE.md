...
- `platform_admin` (master/superadmin) — owns the Peningads API key and the Vercel deployment
...
White-label rule: agencies and their clients must NEVER see "Peningads" or its UI. Peningads is a backend-only data provider for us. The brand the client sees is the agency's brand, fronted by AdSolution.
...
- **Data provider**: Peningads REST API (the upstream host lives at `https://zernio.com/api/v1` — referenced only as the env-default fallback in `lib/peningads/client.ts`; everywhere else we say "Peningads")
...
4. **The agency's brand fronts the client**: white-label means the client portal must never reveal "Peningads" or the upstream stack.
...
app/
  ...
  api/
    cron/sync                   # hourly Peningads → ad_data sync (POST /api/cron/sync)
    cron/anomaly-check          # nightly 1AM UTC alert scan
    cron/whatsapp-daily-digest  # 1:05 AM UTC client digest
    webhooks/peningads          # Peningads webhook receiver (HMAC verified)
...
lib/
  peningads/                    # Peningads integration
    client.ts                   # PeningadsClient — endpoints used:
    ...
    sync.ts                     # syncBrandWindow — chained walk:
                                ...
                                # Throttled to <60 req/min (Peningads rate limit)
    sync-connections.ts         # reconcile Peningads /v1/accounts → brand_ad_accounts
...
supabase/migrations/            # numbered migrations 0001..0015 — apply in order
```
...
- `brands` — agency's clients. One row per brand. Carries `peningads_profile_id` (one Peningads Profile per brand, holds many connected SocialAccounts). Renamed from `zernio_profile_id` in migration 0015.
- `brand_ad_accounts` — connected SocialAccounts (Pages / advertiser tokens). After mig 0009 we only persist ADS-side ones (`metaads` / `tiktokads` / `googleads` Peningads platforms). Organic `facebook` / `tiktok` SocialAccounts stay in Peningads but aren't tracked locally.
...
## Peningads integration — the truth
...
```
SocialAccount (Page)            ← brand_ad_accounts.external_account_id (Peningads _id)
...
PlatformAdAccount (act_123)     ← brand_platform_ad_accounts.platform_ad_account_id
   ...
   ▼
Ad (with summary metrics)       ← uses _id as identifier (NOT id — Peningads returns _id)
...
- **Meta (facebook/instagram)**: same-token. Calling `/v1/connect/facebook/ads` with an existing Page already in Peningads returns `{alreadyConnected: true}` instantly — no second OAuth. Peningads is a Meta Marketing Partner, so the Page token already has ads scope.
...
- **Google**: standalone. `/v1/connect/googleads/ads` returns Google OAuth URL. No MCC application needed — Peningads uses their developer token.

**Webhook to subscribe to** (we have this): `account.ads.initial_sync_completed` fires when Peningads' 90-day ad backfill finishes for a newly-connected ads account. Until that fires, `/v1/ads` returns 0 even on healthy connections. Our webhook handler at `app/api/webhooks/peningads/route.ts` triggers an internal `syncBrandWindow` when this arrives.
...
- We previously read `ad.id` from /v1/ads response — Peningads actually returns `_id`. Use `ad._id ?? ad.id`.
- We previously read `campaignId` / `adSetId` — Peningads uses `platformCampaignId` / `platformAdSetId` on the Ad documents.

---

## MCP servers (configured at `.mcp.json`, gitignored)

Four MCP servers register at Claude Code startup. Note: the data provider's MCP server is registered under its legacy name "zernio" because that's the literal name the upstream MCP host exposes — the tool prefixes below (`mcp__zernio__*`) are external identifiers, not our brand. In our own code / UI we always say "Peningads".
...
4. **zernio** — HTTP MCP at `https://mcp.zernio.com/mcp` (external host kept under its legacy name) with `Authorization: Bearer sk_...` for our master Peningads API key. ~280 tools across `accounts_*`, `profiles_*`, `ads_*`, `connect_*`, `webhooks_*`, etc. The key tools we actually use:
   - `mcp__zernio__profiles_list` — find a brand's Peningads Profile by name
...
- **Peningads Profile**: `6a00c412d32a19d66e5e1b90` "Aqil Fighter Test (AdSolution)" — created via the email-tag dedupe path during register
- **Connected SocialAccount** (same-token reuse — see paywall note below):
  Peningads id `6a00c43d92b3d8e85fb6a556` (platform `facebook`, displayName `Generasi Pintar`, adsStatus `connected`).
...
- **Real spend (Meta side)**: ~RM 9.65 on 2026-02-03 across 3 ads in 2 campaigns
  ...
  only after Peningads' `account.ads.initial_sync_completed` webhook fires.

**Same-token paywall recovery** (commit `065ffb1`): the master Peningads account is
on the free 2-account tier. When `/v1/connect/facebook/ads` would push us over
the cap (because Peningads counts soft-deleted-in-grace accounts toward the limit),
...
2. **Verify a fix end-to-end**:
   ...
   - For Peningads state: use the per-account `mcp__zernio__*` MCP tools (external prefix kept) instead of hitting the live HTTP API
...
4. **Trigger sync from MCP** (no Vercel needed): call `mcp__zernio__connect_ads(platform=facebook, profile_id=<peningads_profile_id>)`. If alreadyConnected:true, the `metaads` SocialAccount exists and you can query `ads_list_ads` directly.

---

## Recent decisions / gotchas (last 7 days)

- **Peningads rebrand** (migration 0015): renamed `lib/zernio/` → `lib/peningads/`, `app/api/webhooks/zernio/` → `app/api/webhooks/peningads/`, `app/api/zernio/` → `app/api/peningads/`. Class `ZernioClient` → `PeningadsClient`, function `getZernio()` → `getPeningads()`. Env vars `ZERNIO_API_KEY` / `ZERNIO_API_BASE_URL` / `ZERNIO_WEBHOOK_SECRET` → `PENINGADS_*` (legacy names still read as fallback so a Vercel deploy with old env vars boots fine). Column `brands.zernio_profile_id` → `brands.peningads_profile_id`. The literal upstream host `https://zernio.com/api/v1` stays only as the env-default fallback in `lib/peningads/client.ts`; the external MCP server stays registered under its literal name `zernio` because the tool prefixes are upstream-defined.
- **Default date range** is **last 90 days** (matches Peningads' discovery backfill). See `lib/client-data/aggregate.ts:parseDateRange`. Test brand spend on 2026-02-03 is just outside the default — extend window manually until we add a "max-back-from-data" auto-default.
...
- **Google Ads via Peningads**: works through `/v1/connect/googleads/ads`. We previously had a "Contact agency" gate — that's wrong; remove it when wiring the UI button.
...
- Webhook `PENINGADS_WEBHOOK_SECRET` env var is empty in Vercel — receiver skips signature verification when missing. Set it + reconfigure the Peningads webhook to use the same secret to harden. (Legacy `ZERNIO_WEBHOOK_SECRET` env var is read as a fallback during the rebrand transition.)
